...
resp <- c("abund.mean","range.size","range.x","range.y","range.loc")
...
for(i in unique(lresi$species)){
  ...
  lresi$range.x[lresi$species==i] <- lresi$range.x[lresi$species==i]/max(abs(lresi$range.x[lresi$species==i]))
  lresi$range.y[lresi$species==i] <- lresi$range.y[lresi$species==i]/max(lresi$range.y[lresi$species==i])
}
