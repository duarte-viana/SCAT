...
gg.time.order <- re.sp %>%
  ...
  ggplot(aes(y = sporder, x = grid_mean)) + 
  stat_halfeyeh(.width = c(0.50, 0.90)) +
  geom_vline(xintercept = 0, linetype = "dashed") +
...
gg.time.threat <- na.exclude(re.sp) %>%
  mutate(grid_mean = coef.r2) %>%
  ggplot(aes(y = redlistCategory, x = grid_mean)) 
  stat_halfeyeh(.width = c(0.50, 0.90)) +
  geom_vline(xintercept = 0, linetype = "dashed") +
...
ggarrange(gg.time.order, gg.time.threat, nrow=1)
...
gg.abund <- re.sp.sum %>%
  ggplot(aes(x=mean.abund, y=mean.r2)) + 
  geom_point(col="darkgrey", size=1) +
  geom_errorbar(aes(xmin=lower.abund, xmax=upper.abund), col="darkgrey",
                position=position_dodge(0.05)) +
  geom_errorbar(aes(ymin=lower.r2, ymax=upper.r2), col="darkgrey",
                position=position_dodge(0.05)) +
  geom_point(data=re.sp.sum[!sig.r2,], aes(x=mean.abund, y=mean.r2), col="red", size=1) +
  geom_point(data=re.sp.sum[!sig.abund,], aes(x=mean.abund, y=mean.r2), col="blue", size=1) +
  geom_point(data=re.sp.sum[sig.r2.abund,], aes(x=mean.abund, y=mean.r2), col="purple", size=1) +
  geom_hline(yintercept = 0, linetype = "dashed", size=1, col="red", alpha = 0.5) +
  geom_vline(xintercept = 0, linetype = "dashed", size=1, col="blue", alpha = 0.5) +
  geom_smooth(method="lm", col="black") +
  labs(x = expression(paste(Delta,"abundance")), y = expression(paste(Delta,"responsiveness to climate")))
...
gg.range.size <- re.sp.sum %>%
  ggplot(aes(x=mean.range.size, y=mean.r2)) + 
  geom_point(col="darkgrey", size=1) +
  geom_errorbar(aes(xmin=lower.range.size, xmax=upper.range.size), col="darkgrey",
                position=position_dodge(0.05)) +
  geom_errorbar(aes(ymin=lower.r2, ymax=upper.r2), col="darkgrey",
                position=position_dodge(0.05)) +
  geom_point(data=re.sp.sum[!sig.r2,], aes(x=mean.range.size, y=mean.r2), col="red", size=1) +
  geom_point(data=re.sp.sum[!sig.rgsi,], aes(x=mean.range.size, y=mean.r2), col="blue", size=1) +
  geom_point(data=re.sp.sum[sig.r2.rgsi,], aes(x=mean.range.size, y=mean.r2), col="purple", size=1) +
  geom_hline(yintercept = 0, linetype = "dashed", size=1, col="red", alpha = 0.5) +
  geom_vline(xintercept = 0, linetype = "dashed", size=1, col="blue", alpha = 0.5) +
  geom_smooth(method="lm", col="black") +
  labs(x = expression(paste(Delta,"range size")), y = expression(paste(Delta,"responsiveness to climate")))

sig.rgy <- re.sp.sum$lower.range.y<=0 & re.sp.sum$upper.range.y>=0
sig.r2.rgy <- sig.r2==FALSE & sig.rgy==FALSE
gg.range.y <- re.sp.sum %>%
  ggplot(aes(x=mean.range.y, y=mean.r2)) + 
  geom_point(col="darkgrey", size=1) +
  geom_errorbar(aes(xmin=lower.range.y, xmax=upper.range.y), col="darkgrey",
                position=position_dodge(0.05)) +
  geom_errorbar(aes(ymin=lower.r2, ymax=upper.r2), col="darkgrey",
                position=position_dodge(0.05)) +
  geom_point(data=re.sp.sum[!sig.r2,], aes(x=mean.range.y, y=mean.r2), col="red", size=1) +
  geom_point(data=re.sp.sum[!sig.rgsi,], aes(x=mean.range.y, y=mean.r2), col="blue", size=1) +
  geom_point(data=re.sp.sum[sig.r2.rgsi,], aes(x=mean.range.y, y=mean.r2), col="purple", size=1) +
  geom_hline(yintercept = 0, linetype = "dashed", size=1, col="red", alpha = 0.5) +
  geom_vline(xintercept = 0, linetype = "dashed", size=1, col="blue", alpha = 0.5) +
  geom_smooth(method="lm", col="black") +
  labs(x = expression(paste(Delta,"range latitude")), y = expression(paste(Delta,"responsiveness to climate")))


quartz(height=3,width=6)
ggarrange(gg.abund, gg.range.size, nrow=1)
